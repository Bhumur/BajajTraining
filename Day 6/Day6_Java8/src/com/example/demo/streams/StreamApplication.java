...
public class StreamApplication {
	...
	public static void getDonorWithHighestAge(List<BloodDonor> list) {
			Stream<Integer> max = list.stream()
						.map(e -> e.getDonerAge());
			Stream<Integer> min = list.stream()
					.map(e -> e.getDonerAge());;
			Optional<Integer> maxAns = max
				.max(Comparator.comparing(Integer::intValue));
			Optional<Integer> minAns = min
					.min(Comparator.comparing(Integer::intValue));
			System.out.println(maxAns.get() + minAns.get());
	}
...
//		getDonorWithHighestAge(donors);
		
		Map<BloodGroup, List<BloodDonor>> map = donors.stream()
		.collect(Collectors.groupingBy(e -> e.getBloodGroup()));
		
//		map.get(BloodGroup.ABNEG).forEach(System.out::println);
		
		Map<BloodGroup, Set<BloodDonor>> set = donors.stream()
				.collect(Collectors.groupingBy(e -> e.getBloodGroup(), Collectors.toSet()));
				
//				map.get(BloodGroup.ABNEG).forEach(System.out::println);
//				set.get(BloodGroup.ABNEG).forEach(System.out::println);
				
				
				LocalDate dpDate = LocalDate.of(2025, 10, 12);
				
				LocalDate c = dpDate.plusDays(45);
